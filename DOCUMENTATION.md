...
* Add it to a channel;
* [Get the Bot User OAuth Token](https://github.com/kasunkv/slack-notification/blob/master/generate-slack-token.md#4-copy-oauth-access-token--use-in-azure-pipelines) e.g. 'xoxb-111-222-xxxxx'. Bot OAuth Token will be stored in the `SLACK_KEY` variable in the file config.inc.sh. The angle brackets around your_key need to be removed.
...
```shell
	...
	source ${HOME}/.bashrc
```
6. Set the crontab to autostart the pipeline. Add to [crontab settings](https://man7.org/linux/man-pages/man5/crontab.5.html) the next lines (keep in mind, that you should replace "gis" with your username):
0 12 * * * /bin/bash /your_working_directory/geocint-runner/start_geocint.sh > /your_working_directory/geocint/log.txt
...
34 12 * * * /bin/bash /your_working_directory/geocint-runner/start_geocint.sh > /your_working_directory/geocint/log.txt
